...
st.set_page_config(page_icon="💲",page_title="DoD Contract Spending Explorer",layout="wide") # Increase page width for app
import pandas as pd
import numpy as np
import asyncio
import aiohttp
...
def breakdown_by(toptier_code,year,breakdown):
    ...
    if response.status_code == 200: # If successful
        ...
        df = df.rename(columns={"name":"Breakdown","obligated_amount":"Obligations"}) # Rename columns
        return df
...
if __name__ == "__main__":
    ...
    st.title('Department of Defense - Contract Data Explorer')

    st.write("""The Department of Defense\'s contract spending exceeds that of practically all other federal agencies.
            USAspending, a service run by the Department of the Treasury, publishes data on contracts as submitted by agencies to the Federal Procurement Data System.""")

    st.markdown("""There are a few different ways that we can consider the federal budget. The government uses the terms *obligations* and *outlays* to describe different
    views of the budget.""")

    tab1, tab2 = st.tabs(['Obligations', 'Outlays'])

    tab1.markdown('<h4 align="left">What are obligations?</h2>', unsafe_allow_html=True)

    obl_text = """Obligations create a legal liability of the part of the government to pay for goods and services.
                An agency incurs an obligation, for example, when it places an order, signs a contract, awards a grant, purchases a service,
                or takes other actions that require the government to make payments to the public or from one government account to another.
                Essentially, they are an agency\'s formal and written commitments to use funds for a specific purpose."""

    tab1.write(obl_text)

    tab2.markdown('<h4 align="left">What are outlays?</h2>', unsafe_allow_html=True)

    outl_text = """Outlays indicate the issuance of checks, disbursement of cash, or electronic transfer of funds made to liquidate a federal obligation.
                Essentially, outlays represent the government actually making a purchase by spending the funds that they have obligated for a specific purpose."""

    tab2.write(outl_text)

    url = 'https://www.cbo.gov/sites/default/files/114th-congress-2015-2016/reports/42904-CBOGlossary.pdf'
    if st.button('Learn more from the Congressional Budget Office\'s Glossary'): # of terms used in the federal budget process
        webbrowser.open_new_tab(url)

    #st.write('Check out the [Federal Spending Transparency Data Standards](https://portal.max.gov/portal/assets/public/offm/DataStandardsFinal.htm) to learn more about obligations and other terminology.')
    st.title('')
    st.subheader('Visualizing budget data')
    view = st.radio('Choose your view:',('Obligations','Outlays'))

    if view == 'Obligations':
        st.markdown('<h4 align="left">Obligations</h2>', unsafe_allow_html=True)
        usasplink = 'https://www.usaspending.gov/'
        st.write(f'Users can pull live data on an agency\'s obligated amounts directly from the Department of the Treasury\'s [USAspending database]({usasplink}) using their API. Their source for this data is the [Report on Budget Execution and Budgetary Resources](https://portal.max.gov/portal/document/SF133/Budget/FACTS%20II%20-%20SF%20133%20Report%20on%20Budget%20Execution%20and%20Budgetary%20Resources.html).')
        st.subheader('')
        ...
        agency_name = st.selectbox("Choose a federal agency to explore their obligation data:", agencies) # Store user selection for agency name
        ...
        if agency_name != ' ': # If agency name has been selected
            ...
            if df.shape[0]==0: # If df of results had 0 rows
                ...
                fig = px.bar(df, x="Fiscal Year", y="Obligations", color="Subagency",title=f'{agency_name} - Obligations by Subagency',color_discrete_sequence=px.colors.qualitative.Prism) # Create plot and set title and colors

                fig.update_xaxes(title_text="Fiscal Year") # Name x axis
                fig.update_yaxes(title_text="Obligations ($)") # Name y axis
                ...
                if agency_name2 != ' ': # If agency name 2 has been selected
                    ...
                    if agency_name2 == agency_name: # If agency name and agency 2 name are the same
                        ...
                        if a1.shape[0]==0 or a2.shape[0]==0: # If results for agency 1 or agency 2 have 0 rows
                            ...
                            fig = px.line(h, x='Fiscal Year', y='Obligations', color='Agency',title=f'Compare Obligations - {agency_name} and {agency_name2}',  color_discrete_sequence=px.colors.qualitative.G10) # Create plot, set title and colors
                            ...
                            h['hoverdata'] = h['Obligations'].apply(human_format)
                            ...
                            st.subheader(f'What did the {agency_name} obligate money for in {default}?') # Add another subheader
                            st.caption('*Category data is only available starting in 2017.')
                            st.write('')

                            st.write('There are two main ways to categorize federal funds.')

                            btab, otab = st.tabs(['Budget Function', 'Object Class'])

                            btab.markdown('<h4 align="left">What is a budget function?</h2>', unsafe_allow_html=True)
                            btab.write('The federal budget is divided into approximately 20 categories, known as budget functions. These categories organize federal spending into topics based on the major purpose the spending serves (e.g., National Defense, Transportation, Health). These are further broken down into budget sub functions.')

                            otab.markdown('<h4 align="left">What is an object class?</h2>', unsafe_allow_html=True)
                            otab.write('Object class is one way to classify financial data in the federal budget. An object class groups obligations by the types of items or services purchased by the federal government. (e.g. Personnel Compensation and Equipment).')

                            with otab.expander('List of object classes'):
                                st.write('List')


                            st.subheader('')
                            select = st.radio("Categorize funds by:",('Budget Function','Object Class')) # Store user's selection of breakdown method from radio buttons
                            ...
                            if b.shape[0]==0: # If results have 0 rows
                                ...
                                b['hoverdata'] = b['Obligations'].apply(human_format)

                                # Create pie chart
                                fig = go.Figure(data=[go.Pie(labels=b['Breakdown'], values=b['Obligations'])]) # Create plot
                                ...
                                fig.update_layout(height=700,font=dict(size=16),showlegend=True,title=f'{agency_name} - Obligation Breakdown by {select}, {year}',title_x=0.5) # Set plot height, font size, title, and center title
                            ...
                            st.subheader(f'What could we pay for with the {agency_name}\'s {year} obligations?') # Add another subheader
                            st.text('(Based on estimates found online)') # And some more text
                            str_year = str(year)
                            spend2021 = a1.loc[a1['Fiscal Year'] == str_year, 'Obligations'].item() # Store agency 1's 2021 spending amount from results of earlier function call (for line graph)
